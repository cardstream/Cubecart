<form action="{$VAL_SELF}" method="post" enctype="multipart/form-data">
	<div id="cardstream" class="tab_content">
		...
		<fieldset><legend>{$LANG.module.cubecart_settings}</legend>
			...
			<div>
				<label for="integration_type">{$LANG.cardstream.integration_type}</label>
				<span>
					<select name="module[integration_type]">
						<option value="hosted" {$SELECT_integration_type_hosted}>{$LANG.cardstream.hosted}</option>
						<option value="hosted_v2" {$SELECT_integration_type_hosted_v2}>{$LANG.cardstream.embedded}</option>
						<option value="hosted_v3" {$SELECT_integration_type_hosted_v3}>{$LANG.cardstream.modal}</option>
					</select>
				</span>
			</div>
